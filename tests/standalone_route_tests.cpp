...
int main() {
    check(
        ps5mc_request_is_launch("GET /launch HTTP/1.1\r\nHost: 127.0.0.1\r\n\r\n"),
        "exact launch route accepted");
    check(!ps5mc_request_is_launch(nullptr), "null rejected");
    check(!ps5mc_request_is_launch(""), "empty request rejected");
    check(
        !ps5mc_request_is_launch("POST /launch HTTP/1.1\r\n\r\n"),
        "alternate method rejected");
    check(
        !ps5mc_request_is_launch("GET /launch?path=/tmp/a HTTP/1.1\r\n\r\n"),
        "query string rejected");
    check(
        !ps5mc_request_is_launch("GET /launcher HTTP/1.1\r\n\r\n"),
