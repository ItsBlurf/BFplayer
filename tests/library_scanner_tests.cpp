...
int main() {
    ...
    check(ps5mc::classify_media_path("Movie.MKV") == MediaKind::video, "MKV classification");
    check(ps5mc::classify_media_path("raw.AV1") == MediaKind::video, "raw AV1 classification");
    check(ps5mc::classify_media_path("capture.WTV") == MediaKind::video, "WTV classification");
    check(ps5mc::classify_media_path("classic.SMK") == MediaKind::video, "Smacker classification");
    check(ps5mc::classify_media_path("song.FLAC") == MediaKind::audio, "FLAC classification");
    check(ps5mc::classify_media_path("audiobook.M4B") == MediaKind::audio, "M4B classification");
    check(ps5mc::classify_media_path("surround.TRUEHD") == MediaKind::audio,
          "TrueHD classification");
    check(ps5mc::classify_media_path("list.m3u8") == MediaKind::playlist, "M3U8 classification");
    check(ps5mc::classify_media_path("captions.ASS") == MediaKind::subtitle, "ASS classification");
    check(ps5mc::classify_media_path("captions.SCC") == MediaKind::subtitle, "SCC classification");
    check(ps5mc::classify_media_path("readme.md") == MediaKind::unknown, "unknown classification");

    std::vector<std::string> episodes{"Episode 10.mkv", "Episode 2.mkv", "Episode 1.mkv"};
    std::sort(episodes.begin(), episodes.end(), ps5mc::natural_path_less);
    ...
    for (std::size_t left = 0; left < comparator_corpus.size(); ++left) {
        ...
        for (std::size_t right = 0; right < comparator_corpus.size(); ++right) {
            const bool left_right = ps5mc::natural_path_less(
                comparator_corpus[left], comparator_corpus[right]);
            const bool right_left = ps5mc::natural_path_less(
    ...
    const auto sidecars = ps5mc::match_subtitle_sidecars("Movie.mkv", candidates);
    ...
    const std::filesystem::path root =
        std::filesystem::temp_directory_path() / "ps5mc-scanner-test";
    ...
    std::vector<ps5mc::MediaEntry> found;
    ...
    const auto scan = ps5mc::scan_media_library(
        root_string,
        [&](const ps5mc::MediaEntry& entry) {
    ...
    check(std::all_of(found.begin(), found.end(), [](const ps5mc::MediaEntry& entry) {
              return entry.size == 4;
          }), "file sizes captured");
    const auto discovered_sidecars = ps5mc::find_subtitle_sidecars(
        root_string + "/Movie.mkv");
    check(discovered_sidecars.size() == 2, "directory sidecar discovery");

    std::vector<ps5mc::MediaEntry> limited_found;
    const auto limited_scan = ps5mc::scan_media_library(
        root_string,
        [&](const ps5mc::MediaEntry& entry) {
            limited_found.push_back(entry);
            return true;
        },
        ps5mc::ScanLimits{32, 100, 1});
    ...
    const auto cancelled_scan = ps5mc::scan_media_library(
        root_string,
        [](const ps5mc::MediaEntry&) { return true; },
        {},
        []() { return true; });
