...
int main() {
    ...
    const fs::path root = fs::temp_directory_path() /
        ("ps5mc-playlist-tests-" + std::to_string(
    ...
    const ps5mc::PlaylistLoadResult m3u_result =
        ps5mc::load_generic_playlist(utf8(m3u));
    ...
    const ps5mc::PlaylistLoadResult pls_result =
        ps5mc::load_generic_playlist(utf8(pls));
    ...
    write_file(
        ...
        "</trackList></playlist>");
    const ps5mc::PlaylistLoadResult xspf_result =
        ps5mc::load_generic_playlist(utf8(xspf));
    ...
    const ps5mc::PlaylistLoadResult truncated =
        ps5mc::load_generic_playlist(utf8(m3u), 1024U * 1024U, 1);
    check(truncated.items.size() == 1 && truncated.truncated,
          "playlist item limit is enforced");
    const ps5mc::PlaylistLoadResult too_small =
        ps5mc::load_generic_playlist(utf8(m3u), 8, 10);
    check(too_small.items.empty() && !too_small.error.empty(),
          "playlist byte limit is enforced");
    check(!ps5mc::is_generic_playlist_path("live.m3u8"),
