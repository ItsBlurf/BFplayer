...
| Network playback | Direct URL input plus cancellable DLNA/UPnP discovery and paged ContentDirectory browsing; bounded HTTP responses/results, strict URL policy, versioned service actions, validated multi-resource selection, FFmpeg reconnect handling, byte-capped compressed queues, and small decoded-frame queues | Host protocol/budget tests, Linux socket/HTTP/SOAP integration tests, and target build verified; live PS5 NAS/network behavior unobserved |
