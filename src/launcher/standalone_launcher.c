...
INCASSET(ps5mc_embedded_player, "dist/ps5-media-center.elf");
...
static int launch_embedded_player(void) {
    ...
    int stdio_descriptor = open(
        ...
        0600);
    const pid_t player_pid = hbldr_launch_buffer(
        PS5MC_RUNTIME_DIR,
        PS5MC_PLAYER_PATH,
        stdio_descriptor,
        argv,
        envp,
        ps5mc_embedded_player);
    if (stdio_descriptor >= 0) {
        close(stdio_descriptor);
    }
    launcher_log(
        "launch result=%s player_pid=%ld embedded_bytes=%lu",
        player_pid > 0 ? "started" : "failed",
        (long)player_pid,
        ps5mc_embedded_player_size);
...
int main(void) {
    ...
    launcher_log(
        ...
        PS5MC_SERVICE_PORT,
        ps5mc_embedded_player_size);
    if (ps5mc_embedded_player_size < 64U ||
        ps5mc_embedded_player[0] != 0x7f ||
        ps5mc_embedded_player[1] != 'E' ||
        ps5mc_embedded_player[2] != 'L' ||
        ps5mc_embedded_player[3] != 'F' ||
        ps5mc_embedded_player[4] != 2U) {
        launcher_log("embedded player validation failed");
        return 2;
    }
